...
**Tool Availability:** `manage_plan create operation`/`manage_plan update operation`/`manage_plan read operation`/`manage_plan list operation` are **CLI-only** — disabled in VSCode extension (`isTTY` check). If these tools error, use `manage_plan capability` for progress tracking. Plan files remain the source of truth; hydration is an optimization, not a requirement.
...
### Phase-Level manage_plan create operation
...
### Critical Step manage_plan create operation
...
2. Cook Step 3: `manage_plan list operation` → empty (tasks died with old session)
3. Cook reads plan files → re-hydrates from unchecked `[ ]` items
4. Already-checked `[x]` items = done, skip those

### Sync-Back (cook Step 6)

1. `manage_plan update operation` marks all session tasks complete.
