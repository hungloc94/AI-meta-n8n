...
Execute AgentKit workflow tests with manual verification at each step.
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/ak-test/scripts/scan-components.py --json --scenarios
```

### Step 2: Load Workflow

Based on `$ARGUMENTS`, load the appropriate workflow from test skill.
...
3. **Show output and ask:**
   - Use ask_user capability: Pass / Fail / Skip
   - Log result
