...
1. **Activate `the engineer project-management skill` skill (MANDATORY)** → sync plan/task status if fix is part of a plan, update progress, hydrate Claude Tasks.
2. Spawn `docs-manager` subagent if API/behavior changed.
3. `manage_plan update operation` to mark Claude Tasks complete.
...
| 5 | Report |
| 6 | `the engineer project-management skill` (MANDATORY), `docs-manager`, `git-manager`, `/ak:journal` |
