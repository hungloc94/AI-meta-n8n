...
Before producing a plan, you MUST be able to answer ALL of these in one concrete sentence each (use `ask_user capability` to pin them down — do NOT proceed on vague intent):
...
Ground every `ask_user capability` option in scout findings (e.g., "Add to `src/api/users.ts` (matches existing pattern) or new `src/api/profile.ts`?"). Skip ONLY when input is a `plan.md`/`phase-*.md` path.
</HARD-GATE-EXACT-REQUIREMENTS>
...
**Claude Tasks:** Utilize `manage_plan capability`, `manage_plan capability`, `manage_plan capability`, `manage_plan capability` during implementation step. **Fallback:** These are CLI-only tools — unavailable in VSCode extension. If they error, use `manage_plan capability` for progress tracking instead.
...
- **Finalize (MANDATORY - never skip):**
  ...
  3. `manage_plan capability` → mark all Claude Tasks complete after sync-back verification (skip if Task tools unavailable)
